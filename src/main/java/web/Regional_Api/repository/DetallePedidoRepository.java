...
public interface DetallePedidoRepository extends JpaRepository<DetallePedido, Integer> {
    ...
    // Los métodos usan consultas explícitas para evitar ambigüedades en rutas de propiedades.
        @Query("SELECT dp FROM DetallePedido dp WHERE dp.pedido.id_pedido = :id_pedido")
    List<DetallePedido> detallesPorPedido(@Param("id_pedido") Integer id_pedido);

        @Query("SELECT dp FROM DetallePedido dp WHERE dp.plato.id_plato = :id_plato")
    List<DetallePedido> detallesPorPlato(@Param("id_plato") Integer id_plato);

        @Query("SELECT SUM(dp.subtotal) FROM DetallePedido dp WHERE dp.pedido.id_pedido = :id_pedido")
