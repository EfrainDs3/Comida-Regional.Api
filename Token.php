<?php
$apiBaseUrl = $_ENV['REGIONAL_API_BASE_URL'] ?? 'http://localhost:2020';
$tokenResult = null;
$errorMessage = null;

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $clienteId = trim($_POST['cliente_id'] ?? '');
    $llaveSecreta = trim($_POST['llave_secreta'] ?? '');

    if ($clienteId === '' || $llaveSecreta === '') {
        $errorMessage = 'Debes ingresar el Cliente ID y la Llave Secreta.';
    } else {
        $payload = json_encode([
...
<div class="overlay">
    <div class="container">
        <header class="hero">
            ...
            <p>
                Usa tu <strong>Cliente ID</strong> y tu <strong>Llave Secreta</strong> para solicitar un token JWT.
        ...
        <section class="form-section">
            ...
            <?php if ($errorMessage): ?>
                <div class="flash error">⚠️ <?php echo htmlspecialchars($errorMessage, ENT_QUOTES, 'UTF-8'); ?></div>
            <?php endif; ?>
            ...
            <?php if ($tokenResult): ?>
                ...
                <div class="token-box">
                    ...
                    <div class="meta">
                        <div><strong>Tipo:</strong> <?php echo htmlspecialchars($tokenResult['tokenType'] ?? 'DEV', ENT_QUOTES, 'UTF-8'); ?></div>
                        <?php if (isset($tokenResult['expiresInMillis'])): ?>
                            <div><strong>Expira en:</strong> <?php echo number_format($tokenResult['expiresInMillis'] / 1000 / 60, 0); ?> minutos</div>
                        <?php endif; ?>
                    </div>
            ...
            <form method="post" autocomplete="off">
                ...
                <input type="text" name="cliente_id" id="cliente_id" placeholder="Ingresa tu Cliente ID" required>

                <label for="llave_secreta">Llave Secreta</label>
                <input type="password" name="llave_secreta" id="llave_secreta" placeholder="Ingresa tu llave secreta" required>
            ...
            <p class="note">
                ...
                <strong>Cliente ID</strong> y la <strong>Llave Secreta</strong> inicial. Guarda la llave en un lugar seguro;
